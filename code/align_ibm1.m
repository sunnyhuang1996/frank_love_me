...
%
  ...
  for line_index = 1:length(eng)
      ...
      [uni_fre, i_fre, j_fre] = unique(fre{line_index});
      %fre_count = sum(bsxfun(@eq, lab_fre(:), 1:max(lab_fre)));
      fre_count = hist(j_fre, 1:max(j_fre));
      [uni_eng, i_eng, j_eng] = unique(eng{line_index});
      ...
      for fre_index = 1:length(uni_fre)
          ...
          for eng_index = 1:length(uni_eng)
              ...
              if ~isfield(tcount, uni_eng{eng_index}) || ~isfield(tcount.(uni_eng{eng_index}), uni_fre{fre_index})
          ...
          for eng_index = 1:length(uni_eng)
              ...
              if isfield(t, uni_eng{eng_index}) && isfield(t.(uni_eng{eng_index}), uni_fre{fre_index})
                 ...
                 if strcmp('third', uni_eng{eng_index})
                    %disp(eng{line_index})
                    %disp(uni_eng)
                    %disp(eng_count)
                    %disp(uni_fre)
                    %disp(fre_count)
                    %disp(((t.(uni_eng{eng_index}).(uni_fre{fre_index}) * fre_word_count  * eng_word_count)))
                    %disp(total.(uni_eng{eng_index}))
                 end
              end
          end
      end
